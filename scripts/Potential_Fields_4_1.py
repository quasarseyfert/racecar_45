...
class PotentialField:
    ...
    def __init__(self, ):
        ...
        self.charge_laser_particle = 0.13
    ...
    def scan_callback(self, msg):
        ...
        kick_x_component = (np.ones(1) * self.charge_forward_boost / self.boost_distance**2.0) * 1.50
	kick_y_component = np.zeros(1)
        ...
        total_x_component = (np.sum(scan_x_components) + kick_x_component) * 0.12
        total_y_component = (np.sum(scan_y_components) + kick_y_component) * 0.12

	self.last_total_y_component = total_y_component
	self.error = self.last_total_y_component - total_y_component

	total_y_component += self.kd * self.error
...
#	command_msg.speed = self.alpha * command_msg.speed + (1.0 - self.alpha) * self.last_cmd

	self.last_cmd = command_msg.speed
	
	print ("Speed: ", command_msg.speed)
