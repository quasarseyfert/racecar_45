...
class PotentialField:
    ...
    def scan_callback(self, msg):
        ...
        kick_x_component = (np.ones(1) * self.charge_forward_boost / self.boost_distance**2.0) * 1.33
	kick_y_component = np.zeros(1)
        ...
        total_x_component = (np.sum(scan_x_components) + kick_x_component) * 0.33
        total_y_component = (np.sum(scan_y_components) + kick_y_component) * 0.33
	...
	if command_msg.drive.speed < 0.2 and command_msg.drive.speed > -0.2:
	    if command_msg.drive.speed < 0:
		command_msg.drive.speed = command_msg.drive.speed - 0.3
	    else:
		command_msg.drive.speed = command_msg.drive.speed + 0.3

	print ("Speed: ", (command_msg.drive.speed * 3000), " Angle: ", (command_msg.drive.steering_angle * 180 / 3.14159))
        # Publish the command
